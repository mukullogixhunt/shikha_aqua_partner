...
public class CitySpinnerAdapter extends ArrayAdapter<CityModel> {

    private final LayoutInflater inflater;

    public CitySpinnerAdapter(@NonNull Context context, @NonNull List<CityModel> items) {
    ...
    private View createViewFromResource(int position, View convertView, ViewGroup parent, int resource) {
        ...
        CityModel customItem = getItem(position);

        // Populate the views using the custom object
        if (customItem != null) {
            binding.tvSpinner.setText(customItem.getmCityName());
